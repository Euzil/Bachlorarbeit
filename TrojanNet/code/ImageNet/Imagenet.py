...
class ImagenetModel:
    ...
    '''
    导入视觉识别模型,三种经典的识别DNN网络模型
    '''
    def construct_model(self, model_name):
        if model_name == 'inception':
            self.model = InceptionV3(weights='imagenet') # 是深度为 48 层的卷积神经网络，该预训练网络可以将图像分类至 1000 个目标类别
            from keras.applications.inception_v3 import preprocess_input, decode_predictions
            self.preprocess_input = preprocess_input
            self.decode_predictions = decode_predictions
        elif model_name == 'resnet':
            self.model = ResNet50(weights='imagenet') # ResNet网络提出了残差网络结构(Residual Network)来减轻退化问题，使用ResNet网络可以实现搭建较深的网络结构（突破1000层）
            from keras.applications.resnet50 import preprocess_input, decode_predictions
            self.preprocess_input = preprocess_input
            self.decode_predictions = decode_predictions
        elif model_name == 'vgg':
            self.model == VGG16(weights='imagenet') # VGGNet突出的贡献是证明了很小的卷积，通过增加网络深度可以有效提高性能
